...
# 2 "<built-in>" 2
# 1 "Include/Convolution.c" 2

# 1 "Include/Convolution.h" 1
...
__attribute__((sdx_kernel("convolution", 0))) void convolution(unsigned char *in_img,
                 unsigned char *out_img);
# 3 "Include/Convolution.c" 2



__attribute__((sdx_kernel("convolution", 0))) void convolution(unsigned char *in_img,
                 unsigned char *out_img) {
#line 30 "C:/Github/HLS_Convolution/Convolution/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=convolution
# 7 "Include/Convolution.c"

#line 7 "C:/Github/HLS_Convolution/Convolution/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=convolution
# 7 "Include/Convolution.c"
...
#pragma HLS INTERFACE s_axilite port=out_img bundle=control
 ...
 char kernel[3][3] = {
    ...
    VITIS_LOOP_22_1: for (int i = 0; i < 128 * 128; i++) {
#pragma HLS PIPELINE
 out_img[i] = 0;
    }

    VITIS_LOOP_27_2: for (int i = 1; i < 128 - 1; i++) {
        VITIS_LOOP_28_3: for (int j = 1; j < 128 - 1; j++) {
#pragma HLS PIPELINE II=1
 short sum = 0;
            VITIS_LOOP_31_4: for (int ki = -1; ki <= 1; ki++) {
                VITIS_LOOP_32_5: for (int kj = -1; kj <= 1; kj++) {
                    sum += kernel[ki + 1][kj + 1] * in_img[((i + ki)*128 + (j + kj))];
                }
            }
            if (sum < 0) sum = 0;
            if (sum > 255) sum = 255;
            out_img[((i)*128 + (j))] = (unsigned char)sum;
        }
    }
}
