...
# 2 "Include/Pollings.c" 2
# 1 "Include/Pollings.h" 1
...
__attribute__((sdx_kernel("pollings", 0))) void pollings(uint8_t in_img[64][64],
              uint8_t max_pool[32][32],
              uint8_t min_pool[32][32],
              uint8_t avg_pool[32][32]);
# 3 "Include/Pollings.c" 2
...
__attribute__((sdx_kernel("pollings", 0))) void pollings(uint8_t in_img[64][64],
              ...
              uint8_t min_pool[32][32],
              uint8_t avg_pool[32][32]) {
#line 30 "C:/Github/HLS_Pollings/Pollings_HLS/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=pollings
# 10 "Include/Pollings.c"
...
#pragma HLS INTERFACE s_axilite port=max_pool bundle=control
#pragma HLS INTERFACE s_axilite port=min_pool bundle=control
#pragma HLS INTERFACE s_axilite port=avg_pool bundle=control

#pragma HLS INTERFACE s_axilite port=return bundle=control

 VITIS_LOOP_24_1: for (int i = 0; i < 32; i++) {
        VITIS_LOOP_25_2: for (int j = 0; j < 32; j++) {
#pragma HLS PIPELINE II=1
 uint8_t max_val = 0;
            uint8_t min_val = 255;
            uint16_t sum = 0;

            VITIS_LOOP_31_3: for (int ki = 0; ki < 2; ki++) {
                VITIS_LOOP_32_4: for (int kj = 0; kj < 2; kj++) {
